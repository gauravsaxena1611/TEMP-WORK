...
A container is essentially a runtime boundary around some code that is being executed or some data that is being stored. The name “container” was chosen because I wanted a name that didn’t imply anything about the physical nature of how that container is executed. For example, a single Java EE server like Apache Tomcat can run multiple web applications inside a single Java Virtual Machine, although each of those web applications is essentially isolated from the others. At development time I might have three web applications running on a single Apache Tomcat server, while each web application may be deployed onto a dedicated Apache Tomcat server in a live environment. In this situation, each web application is a “C4 container”, with the deployment being a separate concern.
...
### [](#web-applications-one-container-or-two)Web applications; one container or two?
...
### [](#is-a-java-jar-c-assembly-dll-module-etc-a-container)Is a Java JAR, C# assembly, DLL, module, etc a container?
...
### [](#should-data-storage-services-be-shown-as-software-systems-or-containers)Should data storage services be shown as software systems or containers?
